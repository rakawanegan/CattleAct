...
def main(cfg: DictConfig) -> None:
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor="val_f1score",  # 監視するメトリクスを指定する
        mode="max",  # monitorで指定したメトリクスを最小化するモデルを最良とする
        save_top_k=1,  # 最良のモデルのチェックポイントを1つだけ保存する
        dirpath="checkpoints_dev",
        filename="action_image" + suffix,
    )
